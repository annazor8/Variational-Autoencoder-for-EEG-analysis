...
for file_name in sorted(edf_files[0:20]):
    ...
    raw_mne = mne.io.read_raw_edf(file_path, preload=False)  # Load the EDF file: NB raw_mne.info['chs'] is the only full of information
    ...
    epochs_mne = mne.make_fixed_length_epochs(raw_mne, duration=4, preload=False, overlap=3) #divide the signal into fixed lenght epoch of 4s with 1 second of overlapping: the overlapping starts from the left side of previous epoch
    del raw_mne
...
"""for indx, combo in enumerate(combinations1): #220 is the max number of combinations
    ...
    # Create dataloader
    train_dataloader = DataLoader(dataset=train_dataset,batch_size = 8, shuffle = True, num_workers=6) #the batch is composed by a single sample, the sessions should be shuffled
    validation_dataloader= DataLoader(dataset=validation_dataset, batch_size =8, shuffle = True, num_workers=6)#shuffle = False to be kept because the order of the trials in a session is important to be maintained

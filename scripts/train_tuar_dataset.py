...
import numpy as np
import pandas as pd
from scripts.tuar_training_utils import leave_one_session_out, reconstruction_metrics
...
def main():
    ...
    for indx, combo in enumerate(combinations1):  # 220 is the max number of combinations
        ...
        train_loss_list, validation_loss_list, epoch_list = train_generic.train(model=model, loss_function=loss_function, optimizer=optimizer,
        ...
        df_train_loss.insert(loc=df_train_loss.shape[1], column='training loss', value=train_loss_list,
                             allow_duplicates=True)
        df_val_loss.insert(loc=df_val_loss.shape[1], column='validation loss', value=validation_loss_list,
                           allow_duplicates=True)
